...
function respond_to_call() {
  ...
  function data(callback) {
    var url = "https://ll.thespacedevs.com/2.0.0/launch/upcoming/";
    var data = JSON.stringify({ Token: "alsfihaqeoiqhgqaengqw12131" });
  ...
  function launchDisplay(data) {
      ...
      for (j = 0; j < 1; j++) {
        if (data.launches[i].rocket.agencies.length > 0) {
          if (data.launches[i].rocket.agencies[j].name === "Lockheed Martin") {
            ...
            data.launches[i].rocket.agencies[j].name ===
            ...
            data.launches[i].rocket.agencies[j].name === "Avio S.p.A"
          ) {
            agencies.push("Arianespace");
          } else if (
            data.launches[i].rocket.agencies[j].name ===
          ...
          rockets[i].indexOf("Soyuz") >= 0 ||
          rockets[i].indexOf("Proton") >= 0
        ) {
          agencies.push("Russia");
    ...
    for (i = 0; i < timestamps.length; i++) {
      ...
      if (
        (timeCheck[i] === "00:00:00" && launchYear[i] == currentYear) ||
        launchMonth[i] == "January"
      ) {
        launchDate.push(
          "NET " + netMonth[i] + " " + netDate[i] + " " + launchYear[i]
        );
        ...
        launchDate.push(
          launchMonth[i] + " " + launchDay[i] + ", " + launchYear[i]
        );
    ...
    for (i = 0; i < westamp.length; i++) {
      ...
      hr = localTimeStart.getHours();
      min = localTimeStart.getMinutes();
      if (min < 10) {
        min = "0" + min;
      }
    ...
    function launchTime(open, close, stamp) {
      if (stamp === 0) {
        ...
        return (
          '<p class="time time-' + (i + 1) + '">Instantaneous: ' + open + "</p>"
        );
      } else {
        return (
          '<p class="time time-' +
    ...
    function webcast(link) {
      if (link !== "") {
        return (
          '<p class="webcast"><a href="' +
          link +
          '" title="Watch this launch!" target="_blank"><i class="fas fa-space-shuttle"></i>  Watch webcast</a></p>'
        );
      } else {
    ...
    for (i = 0; i < data.launches.length; i++) {
      ...
      launchInfo +=
        ...
        '">';
      launchInfo +=
        '<div class="top-info"><p class="agency agency-' +
        (i + 1) +
        '">' +
        agencies[i] +
        "</p>";
      launchInfo +=
        '<p id="message1" class="message message1 hidden"></p><p id="message2" class="message message2 hidden"></p></div>'; //Launch-specific Messages
      launchInfo += webcast(vidLink[i]);

      launchInfo +=
        '<h3 id="blue-text" class="rocket rocket-' +
        (i + 1) +
        '"><span class="holder">' +
        rockets[i] +
        '<span id="display" class="diss"></span></span></h3>';
      launchInfo +=
        '<h3 class="mission mission-' + (i + 1) + '">' + missions[i] + "</h3>";
      launchInfo +=
        '<p class="date date-' +
        (i + 1) +
        '"><i id="blue-text" class="far fa-calendar-alt"></i> &nbsp;' +
        launchDate[i] +
        "</p>";
      launchInfo += launchTime(
        window(windowOpenHr[i], windowOpenMin[i]),
        window(windowCloseHr[i], windowCloseMin[i]),
        wsstamp[i]
      );
      launchInfo +=
        '<p class="launchpad launchpad-' +
        (i + 1) +
        '"><i id="blue-text" class="fas fa-location-arrow"></i> &nbsp;' +
        locations[i] +
        "</p></div>";
  ...
  function paginate() {
    ...
    if (main !== null) {
      main.innerHTML +=
        '<button id="more-launches" class="highlighted"><p><i class="fa fa-bolt"></i> &nbsp;Show All Launches</p></button>';
  ...
  function messages() {
    function ses10() {
      ...
      if (one) {
        one.querySelector(".message1").innerHTML =
          '<a href="http://www.ses.com/4233325/news/2016/22407810" title="First booster to fly a second mission" target="_blank">1st Reused Core!</a>';
    ...
    function osiris() {
      ...
      if (one) {
        one.querySelector(".message1").innerHTML =
          '<a href="http://www.patrick.af.mil/Portals/14/documents/Weather/AV-067%20OSIRIS-REx%20L-3%20Forecast.pdf?ver=2016-09-05-082408-593" title="Weather 70% GO" target="_blank">WX 70% GO</a>';
        one.querySelector(".message1").classList.remove("hidden");
      }
    }
    function soyuzMS2() {
      ...
      if (one) {
        one.querySelector(".message1").innerHTML =
          '<a href="https://www.nasa.gov/mission_pages/station/expeditions/future.html" title="Crew to the International Space Station" target="_blank">Exp. 49 Crew';
        one.querySelector(".message1").classList.remove("hidden");
      }
    }
    function cygnusOA5() {
      ...
      if (one) {
        one.querySelector(".message1").innerHTML =
          '<a href="https://www.orbitalatk.com/news-room/insideOA/AntaresUpdate/default.aspx" title="Return to Flight" target="_blank">Return to Flight</a>';
        one.querySelector(".message1").classList.remove("hidden");
      }
    }
